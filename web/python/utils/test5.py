...
def export_video():
	...
	urls = collection.find({'keywords': '蔡英文', 'status': {'$ne': 0}}, {"_id": 0, "status": 0,'keyword': 0})
	print (type(urls))
	print(urls)
	output_dir = "../../static/视频抓取/蔡英文/"+today+"/"
		...
		if i <= 96:
			try:
				...
				if func==sina:
					...
					x = json.dumps(x)
					print(x)
					y['keywords'] = [y['keywords']]
					title = y['keywords']
					print(title)
					str = y['title']
					if str.find("蔡英文") == -1:
						str = str.replace("蔡", "蔡英文")
					outfile = open(output_dir + str + '.json', 'w+')
					outfile.write(x)
					outfile.write('\n')
